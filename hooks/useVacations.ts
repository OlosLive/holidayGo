...
import { getVacationRepository } from '../lib/repositories';
...
export const useVacations = (): UseVacationsReturn => {
  ...
  const repository = getVacationRepository();
  ...
  const fetchVacations = useCallback(async (filters?: { userId?: string; year?: number; month?: number }) => {
    ...
    const { data, error: fetchError } = await repository.fetch(filters);
  ...
  const toggleVacationDay = useCallback(async (
    ...
    if (existingVacation) {
      ...
      const result = await repository.delete(existingVacation.id);
      ...
      const { data, error: insertError } = await repository.create(newVacation);
  ...
  const addVacationDays = useCallback(async (
    ...
    const { data, error: insertError } = await repository.createMany(newVacations);
  ...
  useEffect(() => {
    if (repository.subscribe) {
      const unsubscribe = repository.subscribe((event, vacation) => {
        if (event === 'INSERT') {
          setVacations((prev) => 
            [...prev, vacation].sort((a, b) => 
              new Date(a.vacation_date).getTime() - new Date(b.vacation_date).getTime()
            )
          );
